<?php

class bannerTagsItemGetListProcessor extends modObjectGetListProcessor
{
    public $objectType = 'bannerTags';
    public $classKey = 'tagsBanner';
    public $defaultSortField = 'id';
    public $defaultSortDirection = 'ASC';
    //public $permission = 'list';


    /**
     * We do a special check of permissions
     * because our objects is not an instances of modAccessibleObject
     *
     * @return boolean|string
     */
    public function beforeQuery()
    {
        if (!$this->checkPermissions()) {
            return $this->modx->lexicon('access_denied');
        }

        return true;
    }


    /**
     * @param xPDOQuery $c
     *
     * @return xPDOQuery
     */
    public function prepareQueryBeforeCount(xPDOQuery $c)
    {
        
        return $c;
    }


    /**
     * @param xPDOObject $object
     *
     * @return array
     */
    public function prepareRow(xPDOObject $object)
    {


        $array = $object->toArray();


        $page = $this->modx->getObject('modResource', $array['category_id']);
        $array['category_id'] = $page->get('pagetitle');





        $array['actions'] = array();

        // Edit
        $array['actions'][] = array(
            'cls' => '',
            'icon' => 'icon icon-edit',
            'title' => $this->modx->lexicon('bannerTags_item_update'),
            'action' => 'updateItem',
            'button' => true,
            'menu' => true,
        );
        $array['actions'][] = array(
            'cls' => '',
            'icon' => 'icon icon-tag',
            'title' => $this->modx->lexicon('bannerTags_item_update'),
            'action' => 'editTags',
            'button' => true,
            'menu' => true,
        );

        if (!$array['active']) {
            $array['actions'][] = array(
                'cls' => '',
                'icon' => 'icon icon-power-off action-green',
                'title' => $this->modx->lexicon('bannerTags_item_enable'),
                'multiple' => $this->modx->lexicon('bannerTags_items_enable'),
                'action' => 'enableItem',
                'button' => true,
                'menu' => true,
            );
        } else {
            $array['actions'][] = array(
                'cls' => '',
                'icon' => 'icon icon-power-off action-gray',
                'title' => $this->modx->lexicon('bannerTags_item_disable'),
                'multiple' => $this->modx->lexicon('bannerTags_items_disable'),
                'action' => 'disableItem',
                'button' => true,
                'menu' => true,
            );
        }

        // Remove
        $array['actions'][] = array(
            'cls' => '',
            'icon' => 'icon icon-trash-o action-red',
            'title' => $this->modx->lexicon('bannerTags_item_remove'),
            'multiple' => $this->modx->lexicon('bannerTags_items_remove'),
            'action' => 'removeItem',
            'button' => true,
            'menu' => true,
        );

        return $array;
    }

}

return 'bannerTagsItemGetListProcessor';